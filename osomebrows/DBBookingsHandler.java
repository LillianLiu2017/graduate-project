...
public class DBBookingsHandler extends SQLiteOpenHelper{
    ...
    public String[] getMyBookingsFromDB(String userName){
        ...
        SQLiteDatabase db = getWritableDatabase();
        String query = "SELECT * FROM " + TABLE_BOOKINGS + " WHERE " + COLUMN_USERNAME + " =\"" + userName + "\"" +
    ...
    public String[] getStaffFromDB(String staffName){

        SQLiteDatabase db = getWritableDatabase();
        String query = "SELECT * FROM " + TABLE_BOOKINGS + " WHERE " + COLUMN_STAFFNAME + " =\"" + staffName + "\"" +
                " ORDER BY " + COLUMN_DATE + " DESC";
        ...
        //loop through the results starting from first row
        while (!cursor.isAfterLast()) {
            if (cursor.getString(cursor.getColumnIndex("staffName")) != null) {
    ...
    public String[] adminBookingsFromDB(){
